...
import { weekDaysLookUpLong, weekDaysLookUpShort } from "@/lib/calendar/week/week.constants";
 ...
 */
export function getArrayOfWeekDaysStringShort(
  dayIndice: WeekDaysZeroBasedType = 1
): WeekDaysStringShortType[] {
  return weekDaysLookUpShort[dayIndice].map(({ dayString }) => dayString);
}
export function getArrayOfWeekDaysStringLong(
  dayIndice: WeekDaysZeroBasedType = 1
): WeekDaysStringLongType[] {
